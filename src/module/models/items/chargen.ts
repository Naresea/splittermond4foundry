...
export interface DistributePointsChoice<T extends ChargenOption>
  extends Choice<T> {
  choiceType: ChoiceType.DistributePoints;
  numPoints: number;
}

export interface MatchMultipleChoice<T extends ChargenOption>
  extends Choice<T> {
  choiceType: ChoiceType.MatchMultiple;
  pointOptions: Array<number>;
}

export function isFixedValueChoice<T extends ChargenOption>(
  v: Choice<T>
): v is FixedValueChoice<T> {
  return v.choiceType === ChoiceType.FixedValue;
}

export function isSelectNOfChoice<T extends ChargenOption>(
  v: Choice<T>
): v is SelectNOfChoice<T> {
  return v.choiceType === ChoiceType.SelectNOf;
}

export function isSelectOneOfChoice<T extends ChargenOption>(
  v: Choice<T>
): v is SelectOneOfChoice<T> {
  return v.choiceType === ChoiceType.SelectOneOf;
}

export function isDistributePointsChoice<T extends ChargenOption>(
  v: Choice<T>
): v is DistributePointsChoice<T> {
  return v.choiceType === ChoiceType.DistributePoints;
}

export function isMatchMultipleChoice<T extends ChargenOption>(
  v: Choice<T>
): v is MatchMultipleChoice<T> {
