...
import {PlayerData, PlayerDataService, RollInfo} from '../services/player-data-service';
...
export class GenesisExportService {
    ...
    private static exportBiography(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        ...
        data.size = Number.isNumeric(actor.data.data.koerpergroesse) ? +actor.data.data.koerpergroesse : null;
        data.eyeColor = actor.data.data.augenfarbe;
        data.weight = Number.isNumeric(actor.data.data.gewicht) ? +actor.data.data.gewicht : null;
        ...
        data.freeExp = actor.data.data.erfahrungGesamt - actor.data.data.erfahrungEingesetzt;
        ...
        data.cultureLores = (cultureItem as Item<Kultur> | undefined)?.data.data.kulturkunde?.split(',').map(s => s.trim());
        data.languages = (cultureItem as Item<Kultur> | undefined)?.data.data.sprache?.split(',').map(s => s.trim());
        data.weaknesses = actor.items.filter(i => i.type === ItemType.Schwaeche).map(i => i.name);
    }

    private static exportAttributes(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
    ...
    private static exportFertigkeiten(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        data.skills = [...playerData.fertigkeiten.tableData, ...playerData.kampfFertigkeiten.tableData, ...playerData.magieFertigkeiten.tableData].map((fertigkeit) => {
            const item: Item<Fertigkeit> | undefined = actor.getOwnedItem(fertigkeit.id) as Item<Fertigkeit> | undefined;
            if (!item || item.type !== ItemType.Fertigkeit) {
                return undefined;
            }
            return {
                name: item.name,
                attribute1: (item.data.data.attributEins === 'STR' ? 'STÄ' : item.data.data.attributEins) as any,
                attribute2: (item.data.data.attributZwei === 'STR' ? 'STÄ' : item.data.data.attributZwei) as any,
                value: fertigkeit.roll ?? 0,
                points: item.data.data.punkte,
                modifier: 0,
                masterships: playerData.meisterschaften.tableData.map(mastery => {
                    const masteryItem = actor.items.get(mastery.id) as Item<Meisterschaft> | undefined;
    ...
    private static exportStaerken(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        data.powers = actor.items.filter(i => i.type === ItemType.Staerke).map((i: Item<Staerke>) => ({
            name: i.name,
    ...
    private static exportZauber(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        data.spells = playerData.zauber.tableData.map((zauber) => {
    ...
    private static exportResourcen(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        data.resources = actor.items.filter(i => i.type === ItemType.Resource).map((i: Item<Resource>) => ({
            name: i.name,
    ...
    private static exportRuestungen(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        data.armors = playerData.ruestungen.tableData.map((ruestung) => {
            ...
            return {
                ...
                features: item.data.data.merkmale.split(',').map(s => s.trim()).map((featName) => ({
                    name: featName
                }))
            }
        }).filter(v => v != null);
    ...
    private static exportSchilde(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        data.shields = playerData.schilde.tableData.map((schild) => {
    ...
    private static exportWaffen(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        data.meleeWeapons = playerData.waffen.tableData.map((waffe) => {
            const item = actor.items.get(waffe.id) as Item<Waffe | undefined>;
            if (!item || item.type !== ItemType.Waffe || item.data.data.reichweite) {
                return undefined;
            }
            return {
                name: item.name,
        ...
        data.longRangeWeapons = playerData.waffen.tableData.map((waffe) => {
            const item = actor.items.get(waffe.id) as Item<Waffe | undefined>;
            if (!item || item.type !== ItemType.Waffe || !item.data.data.reichweite) {
                return undefined;
            }
    ...
    private static exportGegenstaende(actor: TargetActor, playerData: PlayerData, data: GenesisSchema): void {
        data.items = [...playerData.benutzbares.tableData, ...playerData.sonstiges.tableData].map((gegenstand) => {
            const item = actor.items.get(gegenstand.id) as Item<Gegenstand | Benutzbar> | undefined;
            if (!item || (![ItemType.Benutzbar, ItemType.Gegenstand].includes(item.type as any))) {
                return undefined;
            }
            return {
                name: item.name,
    ...
    private static exportMondzeichen(actor: TargetActor, playerData: PlayerData, data: GenesisSchema) {
        const moonsignItem = actor.items.find(i => i.type === ItemType.Mondzeichen) as Item<Mondzeichen> | undefined;
        if (!moonsignItem) {
            return;
        }
        data.moonSign = {
