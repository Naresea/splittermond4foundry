...
export class GenesisImportService {
    ...
    private static importBiography(targetActor: TargetActor, genesisData: GenesisSchema): void {
        const data = targetActor.data.data;
        ...
        data.heldengrad = xpUsed < 100 ? 1 : xpUsed < 200 ? 2 : xpUsed < 300 ? 3 : 4;
    }
    ...
    private static async importItemData(targetActor: TargetActor, genesisData: GenesisSchema): Promise<void> {
        ...
        const kultur: CreateItem<Kultur> = GenesisImportService.getKultur(genesisData);
        const ausbildung: CreateItem<Ausbildung> = GenesisImportService.getAusbildung(genesisData);
        const fertigkeiten: Array<CreateItem<Fertigkeit>> = GenesisImportService.getFertigkeiten(genesisData);
        const meisterschaften: Array<CreateItem<Meisterschaft>> = GenesisImportService.getMeisterschaften(genesisData);
        const schwaechen: Array<CreateItem<Schwaeche>> = GenesisImportService.getSchwaechen(genesisData);
        const staerken: Array<CreateItem<Staerke>> = GenesisImportService.getStaerken(genesisData);
        const resourcen: Array<CreateItem<Resource>> = GenesisImportService.getResourcen(genesisData);
        const zauber: Array<CreateItem<Zauber>> = GenesisImportService.getZauber(genesisData);
        const gegenstaende: Array<CreateItem<Gegenstand>> = GenesisImportService.getItems(genesisData);
    ...
    private static getRasse(genesisData: GenesisSchema): CreateItem<Rasse> {
        ...
        const gk = genesisData.attributes.find(a => a.shortName === 'GK')?.value ?? 0;
        return {
            name: raceName,
            type: ItemType.Rasse,
            data: {
                modifier: [
                    { type: ModifierType.Attribute, target: 'GK', value: gk }
                ]
            }
        };
    }

    private static getAbstammung(genesisData: GenesisSchema): CreateItem<Abstammung> {
    ...
    private static getAusbildung(genesisData: GenesisSchema): CreateItem<Ausbildung> {
    ...
    private static getFertigkeiten(genesisData: GenesisSchema): Array<CreateItem<Fertigkeit>> {
    ...
    private static getMeisterschaften(genesisData: GenesisSchema): Array<CreateItem<Meisterschaft>> {
        if (!genesisData.skills) {
            return [];
        }
        return genesisData.skills.map((skill, idx) =>
            skill.masterships.map(mastery => ({
                ...
                data: {
                    ...
                    beschreibung: `<p>${mastery.shortDescription ?? ''}</p><p>${mastery.longDescription ?? ''}</p>`,
    ...
    private static getStaerken(genesisData: GenesisSchema): Array<CreateItem<Staerke>> {
        ...
        return genesisData.powers.map((power, idx) => ({
            ...
            data: {
                beschreibung: `<p>${power.shortDescription ?? ''}</p><p>${power.longDescription ?? ''}</p>`,
    ...
    private static getSchwaechen(genesisData: GenesisSchema): Array<CreateItem<Schwaeche>> {
    ...
    private static getResourcen(genesisData: GenesisSchema): Array<CreateItem<Resource>> {
    ...
    private static getZauber(genesisData: GenesisSchema): Array<CreateItem<Zauber>> {
        if (!genesisData.spells) {
            return [];
        }
        return genesisData.spells.map((spell, idx) => {

            const {erschoepft, verzehrt, kanalisiert} = CalculationService.fromEKVString(spell.focus ?? '0');
    ...
    private static getItems(genesisData: GenesisSchema): Array<CreateItem<Gegenstand>> {
    ...
    private static getSchilde(genesisData: GenesisSchema): Array<CreateItem<Schild>> {
    ...
    private static getRuestungen(genesisData: GenesisSchema): Array<CreateItem<Ruestung>> {
