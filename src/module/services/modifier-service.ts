...
export class ModifierService {
  public static getModifiers(actor: Actor): Modifiers {
    ...
    ModifierService.addMods(
      ...
      sorted,
      (v: Meisterschaft) => v.modifier
    );
    ModifierService.addMods(
      result,
      ItemType.Merkmal,
      sorted,
      (v: Merkmal) => v.modifier
    );
    ModifierService.addMods(
      result,
      ItemType.Schwaeche,
      sorted,
      (v: Schwaeche) => v.modifier
    );
    ModifierService.addMods(
      result,
      ItemType.Staerke,
      sorted,
      (v: Staerke) => v.modifier
    );
    ModifierService.addMods(
      result,
      ItemType.Zustand,
      sorted,
      (v: Zustand) => v.modifier
    );
    ModifierService.addMods(result, ItemType.Ruestung, sorted, (v: Ruestung) =>
  ...
  public static totalMod(
    ...
    if (opts) {
      modifiers = modifiers.filter((mod) => {
        return (
          (opts?.itemType == null || opts?.itemType === mod.source.type) &&
  ...
  private static addMods(
    ...
    const modifiers: Array<DecoratedModifier> = ModifierService.flatten(
      items.map((i) =>
        getModifierField(i.data.data).map((m) => ({
          modifier: m,
          source: i,
        }))
      )
    ...
    modifiers.forEach((modifier) => {
      ...
      const byItemType =
        mods.byItemType.get(modifier.source.type as ItemType) ?? [];
  ...
  private static getRuestungModifier(
    v: Ruestung,
