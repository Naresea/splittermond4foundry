...
export class ChargenMatchMultiple extends FormApplication<
  MatchMultipleChoice<ChargenOption>
> {
  ...
  getData(opt?: object): any | Promise<any> {
    ...
    const canSubmit =
      ...
      Math.min(this.object.pointOptions.length, this.object.options.length);
  ...
  protected activateListeners(html: JQuery<HTMLElement> | HTMLElement): void {
    ...
    html.find(".btn-submit").on("click", (evt) => {
      const selectedOptions = this.confirmedMatches.filter(
        (o) => o.points != null && o.points !== 0
      );
