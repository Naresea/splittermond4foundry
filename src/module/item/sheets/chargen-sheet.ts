...
export class ChargenSheet {
    ...
    const choicesForTemplate = choices.map((choice) => ({
      fields: [
        ...
        `${choice.options?.map((o) =>
          game.i18n.localize(`splittermond.chargen-option-label.type.${o.type}`)
        )}`,
  ...
  private static openChoiceSheet(
    choices,
    index: number,
    updateCallback: (choices: Array<Choice<ChargenOption>>) => void
  ): void {
