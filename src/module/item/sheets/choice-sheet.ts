...
export class ChoiceSheet extends FormApplication<Choice<ChargenOption>> {
  ...
  getData(options?: object): any | Promise<any> {
    return {
      ...
      pointOptions: (
        (this.object as MatchMultipleChoice<ChargenOption>).pointOptions ?? []
      ).join(", "),
  ...
  private doUpdate(formData: any): Promise<any> {
    ...
    (this.object as any).numPoints =
      formData.numPoints ?? (this.object as any).numPoints;
    (this.object as any).pointOptions =
      this.parsePointOptions(formData.pointOptions) ??
      (this.object as any).pointOptions;
    this.render();
    return this.update
  ...
  private openChargenOptionSheet(
    options: Array<ChargenOption>,
    index: number
  ): void {
