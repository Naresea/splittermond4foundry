...
export class AusbildungSheet extends SplimoItemSheet<Ausbildung> {
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            template: 'systems/splittermond/templates/sheets/item/ausbildung-sheet.hbs',
            width: 512,
            height: 766,
            tabs: [
    ...
    getData(): ItemSheet.Data<Ausbildung> {
    ...
    private registerCreateModifierClick(html: JQuery<HTMLElement>): void {
        html.find('.ausbildung-modifier .add-item').on('click', async () => {
    ...
    private registerEditModifierClick(html: JQuery<HTMLElement>): void {
        html.find('.ausbildung-modifier .edit-item').on('click', (evt) => {
    ...
    private registerDeleteModifierClick(html: JQuery<HTMLElement>): void {
        html.find('.ausbildung-modifier .delete-item').on('click', (evt) => {
                const index = +evt.target.dataset.index;
