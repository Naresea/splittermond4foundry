...
export class SchwaecheSheet extends SplimoItemSheet<Schwaeche> {
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            classes: ['splittermond'],
            template: 'systems/splittermond/templates/sheets/item/schwaeche-sheet.hbs',
            width: 512,
            height: 766,
            tabs: [
    ...
    getData(): ItemSheet.Data<Schwaeche> {
    ...
    private registerCreateModifierClick(html: JQuery<HTMLElement>): void {
        html.find('.schwaeche-modifier .add-item').on('click', async () => {
    ...
    private registerEditModifierClick(html: JQuery<HTMLElement>): void {
        html.find('.schwaeche-modifier .edit-item').on('click', (evt) => {
    ...
    private registerDeleteModifierClick(html: JQuery<HTMLElement>): void {
        html.find('.schwaeche-modifier .delete-item').on('click', (evt) => {
                const index = +evt.target.dataset.index;
